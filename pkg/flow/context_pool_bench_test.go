...
func BenchmarkContextWithRequestGroup(b *testing.B) {
	b.Run("pool_enabled", func(b *testing.B) {
		prev := UseContextPool
		UseContextPool = true
		defer func() { UseContextPool = prev }()
		...
		for i := 0; i < b.N; i++ {
			c := NewContext(nil, nil, nil)
			// touch the RequestGroup but do not spawn goroutines to keep the
			// benchmark focused on allocation differences.
			_ = c.RequestGroup()
			PutContext(c)
		}
	})
